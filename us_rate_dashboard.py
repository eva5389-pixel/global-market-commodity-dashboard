...
def _rate_inputs() -> dict[str, pd.DataFrame]:
    # These independent requests run together so opening the tab does not wait
    # for six sequential network round trips.
    with ThreadPoolExecutor(max_workers=len(FRED_SERIES)) as pool:
        futures = {
            name: pool.submit(_fred, code, name)
...
def render_us_rate_dashboard(default_rate: float = 4.0) -> None:
    ...
    try:
        ...
        claims_4w = float(data["初領失業金"]["初領失業金"].tail(4).mean() / 1000)
        effective_rate = _last(data["有效聯邦基金利率"], "有效聯邦基金利率")
        source_ok = True
    except Exception:
        source_ok = False
    ...
    labor_tightness = _clip_score(
        50 + (4.2 - unemployment) * 16 + (payroll_3m - 100) * 0.08 + (230 - claims_4w) * 0.12
    ) if source_ok else 50.0
    ...
    if not source_ok:
        st.warning("FRED資料暫時無法取得；目前僅顯示OIS輸入結果，CPI與就業分數使用中性值。")
        return
    ...
    k4.metric("非農3月均增", f"{payroll_3m:,.0f}千人", f"初領4週均 {claims_4w:,.0f}千", border=True)
    ...
    st.markdown("[FRED CPI](https://fred.stlouisfed.org/series/CPIAUCSL)｜[FRED核心CPI](https://fred.stlouisfed.org/series/CPILFESL)｜[FRED失業率](https://fred.stlouisfed.org/series/UNRATE)｜[FRED非農就業](https://fred.stlouisfed.org/series/PAYEMS)｜[FRED初領失業金](https://fred.stlouisfed.org/series/ICSA)｜[New York Fed SOFR](https://www.newyorkfed.org/markets/reference-rates/sofr)")
